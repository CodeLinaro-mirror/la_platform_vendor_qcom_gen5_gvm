...
ifeq ($(filter $(PLATFORM_VERSION), 16 Baklava),$(PLATFORM_VERSION))
...
ifeq ($(KERNEL_DEFCONFIG),)
    ifeq ($(TARGET_BUILD_VARIANT),user)
        KERNEL_DEFCONFIG := autoghgvm-qgki_defconfig
    else
        KERNEL_DEFCONFIG := autoghgvm-qgki-debug_defconfig
    endif
endif
...
BOARD_BOOTCONFIG := androidboot.hardware=qcom androidboot.selinux=enforcing androidboot.memcg=1 androidboot.recover_usb=1 androidboot.usbcontroller=a200000.dwc3
...
ifeq ($(filter $(PLATFORM_VERSION), UpsideDownCake U 14),$(PLATFORM_VERSION))
